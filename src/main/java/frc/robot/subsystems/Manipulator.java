...
public class Manipulator extends AdvancedSubsystem {
  ...
  private final DigitalInput _manipulatorBeam = new DigitalInput(ManipulatorConstants.manipulatorBeamPort);
  private final DigitalInput _manipulatorSwitch = new DigitalInput(ManipulatorConstants.manipulatorSwitchPort);
  ...
  public Manipulator(Consumer<Piece> currentPieceSetter) {
    ...
    _beamBroken.and(() -> getCurrentPiece() == Piece.NONE).onFalse(runOnce(() -> _currentPieceSetter.accept(Piece.CORAL)));
    _beamBroken.and(() -> getCurrentPiece() != Piece.NONE).onFalse(runOnce(() -> _currentPieceSetter.accept(Piece.NONE)));
